<template>
	...
	<view class="container">
		<uni-section title="练习题" type="line" padding color="#2d1add" />
			<view style="font-size: 12px; color: #e71111; margin-left: 15rpx;">Note: 点击二级标题即可做题</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 跳转到最终学习的页面
			toExercisePage(id, secondName, pname, firstName, pid) {
				const fullName = pname + '(' + firstName + ')'
				const url =  `./exercisePage?bid=${id}&name=${secondName}&pid=${pid}&fullName=${fullName}`
				if (!this.hasLogin) {
					this.login(url)
				} else {
					uni.navigateTo({
